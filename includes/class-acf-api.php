...
if ( ! class_exists( 'ACF_REST_Bridge_ACF_API' ) ) {
	class ACF_REST_Bridge_ACF_API {
		protected $id            = null;
		protected $type          = null;
		...
		protected function format_id() {
			if ( $this->id ) {
				switch ( $this->type ) {
					...
					case 'user':
						...
						break;
					default:
						if ( 'ACF_REST_Bridge_Terms_Controller' === $this->controller ) {
			...
			$this->id = apply_filters( 'acf_rest_bridge/id', $this->id, $this->type, $this->controller );
		...
		public function get_fields( $request ) {
			...
			return apply_filters( 'acf_rest_bridge/' . $this->type . '/get_fields', $data, $request );
		}
